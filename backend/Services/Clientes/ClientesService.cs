...
public class ClientesService(IClientesRepository clientesRepository, IConfiguration configuration, IMapper mapper)
    : IClientesService
{
    public async Task<ResultApiResponse<List<ClienteResponse>>> GetAllClientes(ClienteSort? sortBy = null,
        bool desc = false, int items = 10, int offset = 0)
    {
    ...
    public async Task<ClienteResponse> CreateCliente(ClienteRequest cliente)
    {
        ...
        return cli!=null ? mapper.ClienteEntityToResponse(cli) 
    ...
    public async Task UpdateCliente(Guid id, ClienteRequest cliente)
    {
        ...
        if (exist is not null && exist.Id != cli.Id)
        {
            throw new MiddlewareException(HttpStatusCode.BadRequest,
                new { message = "El email ya pertenece a otro cliente" });
        }
