...
{
    public async Task<IEnumerable<Cliente>> GetClientes() => await context.Clientes.ToListAsync();

    public async Task<Cliente?> GetCliente(int id) => await context.Clientes.FirstOrDefaultAsync(c => c.Id == id);
    public async Task<Cliente?> FindByEmail(string email)=> await context.Clientes.FirstOrDefaultAsync(c => c.Email == email);
